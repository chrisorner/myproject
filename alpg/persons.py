...
class Person:
    ...
    def generate(self, age):
        ...
        self.WeekendActivities = random.randint(config.personWeekendActivityChanceMin,
    ...
    def simulateWorkday(self, day):
        # select variables
        eventList = []
        self.WorkdayWakeUp = random.randint((self.WorkdayWakeUp_Avg - self.WorkdayWakeUp_Variate),
                                            (self.WorkdayWakeUp_Avg + self.WorkdayWakeUp_Variate))
        eventList.append(self.WorkdayWakeUp)
        self.WorkdayLeave = random.randint((self.WorkdayLeave_Avg - self.WorkdayLeave_Variate),
                                           (self.WorkdayLeave_Avg + self.WorkdayLeave_Variate))
        eventList.append(self.WorkdayLeave)
        self.WorkdayArrival = random.randint((self.WorkdayArrival_Avg - self.WorkdayArrival_Variate),
                                             (self.WorkdayArrival_Avg + self.WorkdayArrival_Variate))
        ...
        if ((day % 7) == self.WorkdaySportday):
            ...
            self.WorkdayActivity = random.randint((self.WorkdaySport_Avg - self.WorkdaySport_Variate),
                                                  (self.WorkdaySport_Avg + self.WorkdaySport_Variate))
            eventList.append(self.WorkdayActivity)
            self.WorkdayActivityEnd = self.WorkdayActivity + random.randint(
                (self.WorkdaySportDuration_Avg - self.WorkdaySportDuration_Variate),
                (self.WorkdaySportDuration_Avg + self.WorkdaySportDuration_Variate))
        ...
        self.WorkdayBedTime = min(1439, random.randint((self.WorkdayBedTime_Avg - self.WorkdayBedTime_Variate),
                                                       (self.WorkdayBedTime_Avg + self.WorkdayBedTime_Variate)))
    ...
    def simulateWeekend(self, day):
        ...
        if ((day % 7) == 0 or (day % 7) == 6):
            self.WeekendWakeUp = random.randint((self.WeekendWakeUp_Avg - self.WeekendWakeUp_Variate),
                                                (self.WeekendWakeUp_Avg + self.WeekendWakeUp_Variate))
        else:
            # Day off, get out of bed earlier
            self.WeekendWakeUp = random.randint((self.WeekendWakeUp_Avg - self.WeekendWakeUp_Variate - 60),
                                                (self.WeekendWakeUp_Avg + self.WeekendWakeUp_Variate - 60))
        eventList.append(self.WeekendWakeUp)

        if (((day % 7) == self.WeekendSportday)):
            # Today this person will go to sport or have an activity. Times are synchronized to keep it easy
            self.WeekendActivity = random.randint((self.WeekendSport_Avg - self.WeekendSport_Variate),
                                                  (self.WeekendSport_Avg + self.WeekendSport_Variate))
            eventList.append(self.WeekendActivity)
            self.WeekendActivityEnd = self.WeekendActivity + random.randint(
                (self.WeekendSportDuration_Avg - self.WeekendSportDuration_Variate),
                (self.WeekendSportDuration_Avg + self.WeekendSportDuration_Variate))
        ...
        self.WeekendBedTime = random.randint((self.WeekendBedTime_Avg - self.WeekendBedTime_Variate),
                                             (self.WeekendBedTime_Avg + self.WeekendBedTime_Variate))
...
class PersonWorker(Person):
    def __init__(self, age):
        self.generate(age)
        if age > 55 or random.randint(0,
                                      2) == 0:  # Older people can get a day off sometimes, such as BAPO in the education. Furthermore 33% has a home working day: http://www.kamer033.nl/nieuws/in-8-tips-een-productieve-thuiswerkdag/
