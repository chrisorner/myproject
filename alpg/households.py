...
class Household:
    ...
    def generate(self):
        ...
        #																										   (PH)EV, Heat Pump, PV
        self.ConsumptionYearly = profilegentools.gaussMinMax(config.yearlyConsumption, 500)  # kWh
    ...
    def scaleProfile(self):
        ...
        for k, v, in self.ConsumptionShare.items():
            # k is string "Electronics", "Lighting", "Standby"
            sumDevice = sum(self.consumptionFactor[k])
            multiplier = ((self.ConsumptionShare[k] / 100) * (
                    ((self.ConsumptionYearly / 365) * config.numDays) * 1000) * 60) / sumDevice  # joules
    ...
    def reactivePowerProfile(self):
        ...
        for k, v, in self.ReactiveFactor.items():
            ...
            self.ReactiveConsumption['Total'] = [sum(x) for x in
                                                 zip(self.ReactiveConsumption['Total'], self.ReactiveConsumption[k])]
            ...
            if i in self.WashingDays:
                ...
                if notWorking and random.random() < 0.8:
                    ...
                    if (moment < 0.2):
                        ...
                        self.washingMoment[i] = self.Persons[0].WorkdayWakeUp_Avg + self.Persons[
                            0].WorkdayWakeUp_Variate + 20
                    elif (moment < 0.8):
                        # Evening
    ...
    def generateDishwashdays(self, days):
        ...
        for i in range(0, 7):
            if i in self.DishwashDays:
                ...
                if (moment < 0.2):
                    # Washing in the morning
                    self.DishwashMoment[i] = self.Persons[0].WorkdayWakeUp_Avg + self.Persons[
                        0].WorkdayWakeUp_Variate + 20
                elif (moment < 0.7):
                    # Evening
    ...
    def simulate(self):
        for day in range(config.startDay, config.numDays + config.startDay):
            ...
            DHWDemandProfile = self.HeatingDevices["DHWDemand"].simulate(self.Persons, self.OccupancyPerson, dayOfWeek,
                                                                         cookingTime, cookingDuration,
                                                                         self.hasDishwasher)

            # Kitchen
            if startCooking != -1:
                OtherProfile = self.Devices["Cooking"].simulate(1440, self.OccupancyAdultsDay, self.Persons,
                                                                startCooking, cookingDuration, self.hasInductionCooking,
                                                                self.HeatingDevices["VentFlow"])
            OtherProfile = [sum(x) for x in
                            zip(OtherProfile, self.Devices['Kettle'].simulate(1440, self.OccupancyPersonsDay))]
            ...
            if random.randint(1, 7) == 1:
                OtherProfile = [sum(x) for x in zip(OtherProfile,
                                                    self.Devices["Ironing"].simulate(1440, self.OccupancyAdultsDay,
                                                                                     len(self.Persons)))]

            # Vacuumcleaning
            if random.randint(1, 7) == 1:
                OtherProfile = [sum(x) for x in zip(OtherProfile, self.Devices["Vacuumcleaner"].simulate(1440,
                                                                                                         self.OccupancyAdultsDay,
                                                                                                         len(
                                                                                                             self.Persons)))]
            ...
            if day - config.startDay < config.numDays - 1:
                ...
                if (((dayOfWeek in self.WashingDays) and (random.random() < 0.9)) or (random.random() < 0.1)):
                    self.Devices["WashingMachine"].simulate(1440, day, self.OccupancyAdultsDay,
                                                            self.washingMoment[dayOfWeek])
                ...
                if (self.hasDishwasher == True):
                    if (((dayOfWeek in self.DishwashDays) and (random.random() < 0.9)) or (random.random() < 0.1)):
                        self.Devices["DishwashMachine"].simulate(1440, day, self.OccupancyAdultsDay,
                                                                 self.DishwashMoment[dayOfWeek])

            # Simulate individual devices
            LightingProfile = self.Devices["Lighting"].simulate(1440, self.OccupancyPersonsDay,
                                                                1388534400 + (3600 * 24 * day))
            ElectronicsProfile = self.Devices["Electronics"].simulate(1440, self.OccupancyPersonsDay,
                                                                      self.OccupancyPerson)
        ...
        if self.House.hasPV:
            ...
            self.PVProfile = self.Devices['PVPanel'].simulate(config.startDay, config.numDays * ((3600 * 24) / 60),
                                                              self.House.pvArea, self.House.pvEfficiency,
                                                              self.House.pvAzimuth, self.House.pvElevation)
        else:
...
class HouseholdSingleWorker(Household):
    def __init__(self):
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(config.yearlyConsumption,
                                                             400) * config.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
        ...
        self.Persons[0].setDistanceToWork(config.distancetoWork)
...
class HouseholdCouple(Household):
    ...
    def __init__(self, parttime=False, jobless=False):
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(config.yearlyConsumption,
                                                             700) * config.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
...
class HouseholdFamilyDualParent(Household):
    ...
    def __init__(self, parttime=False, jobless=False):
        self.generate()
        numKids =  config.numKids  # http://www.cbs.nl/nl-NL/menu/themas/bevolking/faq/specifiek/faq-hoeveel-kinderen.htm

        self.ConsumptionYearly = profilegentools.gaussMinMax(config.yearlyConsumption + (700 * numKids), 500 + (
                numKids * 100)) * config.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
        ...
        self.Fridges = [
            devices.DeviceFridge(random.randint(config.ConsumptionFridgeSmallMin, config.ConsumptionFridgeSmallMax)),
            devices.DeviceFridge(random.randint(config.ConsumptionFridgeSmallMin, config.ConsumptionFridgeSmallMax))]
...
class HouseholdDualRetired(Household):
    def __init__(self):
        self.generate()
        self.ConsumptionYearly = profilegentools.gaussMinMax(config.yearlyConsumption,
                                                             600) * config.consumptionFactor  # kWh http://www.nibud.nl/uitgaven/huishouden/gas-elektriciteit-en-water.html
